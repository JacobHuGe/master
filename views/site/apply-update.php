...
<body>
    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]) ?>
        ...
        <div class="weui-cells weui-cells_form">
            ...
            <?php endif; ?>
            <?= $form->field($apply, 'hiddenInput')->hiddenInput(["value" => ""],['autofocus' => true])->label(false) ?>
        </div>
    ...
    <script>
		...
		var id = $(this).siblings('[data-id]').data('id');
                ...
                var dataValue = $('#applyform-hiddeninput').val();
                ...
                if(dataValue){
                    var suffix = dataValue.split(",");
                    var is = 0;
                    var data = '';
                    for(var i=0; i < suffix.length - 1; i++){
                        //alert(suffix)
                        var num = suffix[i].indexOf("=");
                        var studyId = suffix[i].substring(0, num);
                        if(studyId == id){
                            data = data + studyId + "=" + val + ",";
                            is = 1;
                        } else {
                            //alert(suffix + "xxxss")
                            data = data + suffix[i] + ",";
                    ...
                    dataValue = data;
                    $('#applyform-hiddeninput').val(dataValue);
                    //var suffix = dataValue.substr(dataValue.lastIndexOf(","));
                } else {
